...
/*
 *  SODA initializer
 *  initializes project model using other initializers and then applies soda
 *  creation of requirement clusters as well as their subgraphs
 *  calculation of sodaspecific values and transformation into characteristics
 */
    ...
    if (useSoda && (![self sodaValuesAvailable])) {
        ...
        for (Component *oneComponent in self.project.consistsOf) {
            ...
            if (!found) {
                ...
                [Characteristic addNewCharacteristic:@"Autonomy of requirements within this component" withValue:valueCohesion toSuperCharacteristic:@"Communication Complexity" withWeight:[NSNumber numberWithInt:3] andComponent:oneComponent.componentID andProject:oneComponent.partOf.projectID andManagedObjectContext:self.managedObjectContext];
                [Characteristic addNewCharacteristic:@"Number of inter-component requirements links" withValue:valueCoupling toSuperCharacteristic:@"Communication Complexity" withWeight:[NSNumber numberWithInt:3] andComponent:oneComponent.componentID andProject:oneComponent.partOf.projectID andManagedObjectContext:self.managedObjectContext];
            }
...
- (ProjectModel *)initWithProjectID:(NSString *)idOfProject
{
    ...
    //else, initialize model from webservice
        ...
        self.project = [self downloadProjectWithID:idOfProject fromPlatformtoManagedObjectContext:self.managedObjectContext];
 ...
 *
 */
- (Project *)downloadProjectWithID:(NSString *)projectID fromPlatformtoManagedObjectContext:(NSManagedObjectContext *)context
{
...
//returns supercharacteristics used in first component of the proeject --> used in all components
...
//returns the component object for the passed component id
...
//save value for supercharacteristic - used during rating
 ...
 *  returns array of components based on classification:
 *  index 0: Core Components, Index 1: Indifferent, Index 2: Outsourcing
 *
 */
...
# pragma mark Characteristic Consistency

/*
 *  applies unused available characteristics to the project and deletes 
 *  used characteristics that are no longer present in AvailableCharacteristics
 *
 */
...
/*
 *  returns true if there are AvailableSupercharacteristics or AvailableCharacteristics in the core database
 *  that have not been applied as Supercharacteristics or Characteristics in the current project.
 *  comparison is made based on the name
 *
 */
- (BOOL)ratingCharacteristicsHaveBeenAdded
{
    ...
    for (AvailableSuperCharacteristic *availableSuperChar in availableSuperCharacteristics) {
        //check if all available supercharacteristics have been used in the component
        bool foundAvailableSuperChar = NO;
        //iterate used supercharacteristics
        for (SuperCharacteristic *usedSuperChar in component.ratedBy) {
            if ([usedSuperChar.name isEqualToString:availableSuperChar.name]) {
                //found
                foundAvailableSuperChar = YES;
                //check if all available characteristics of a used supercharacteristics have been used
                for (AvailableCharacteristic *availableCharacteristic in availableSuperChar.availableSuperCharacteristicOf) {
                    bool foundAvailableChar = NO;
                    //iterate
                    for (Characteristic *usedCharacteristic in usedSuperChar.superCharacteristicOf) {
                        if ([usedCharacteristic.name isEqualToString:availableCharacteristic.name]) {
                            //found
                            foundAvailableChar = YES;
                        }
                    }
                    if (!foundAvailableChar) {
                        return YES;
...
/*
 *  returns true if there are applied Supercharacteristics or Characteristics in the current project
 *  that are not present in the core database in the form of AvailableSupercharacteristics or 
            ...
            if ([usedSuperChar.name isEqualToString:availableSuperChar.name]) {
                ...
                for (Characteristic *usedCharacteristic in usedSuperChar.superCharacteristicOf) {
                    ...
                    if (!foundCharacteristic) {
                        ...
                        if (!([usedCharacteristic.name isEqualToString:@"Autonomy of requirements within this component"] ||
                              [usedCharacteristic.name isEqualToString:@"Number of inter-component requirements links"])) {
                            return YES;
                        }
                    }
                }
 ...
 *  first dimension references the columns of the decision table
 *      - the last column of the decision table does not cover a supercharacteristic, but the result that the lines (combination
            of supercharacteristics would have produced)
 *  the second dimension differentiates between
 *      - Supercharacteristic name at index 0 - NSString
 *      - Supercharacteristic weight at index 1 - NSNumber
 *      - NSArray of possible substatiations (1, 2, 3) of the supercharacteristic --> third dimension
 *         every index of this array references one cell of the table in the current column
 *  the third dimesnion referneces substantiations in the form of nsnumbers
 *
 */
- (NSArray *)getColumnsForDecisionTable
{
    //arbitary component of project -> characteristics and weightings consistent
...
//checks if soda values have been previously calculated
...
/*
 *  uses the webservice connector to downlaod all requirements associated with the current 
 *  project from the collaboration platform to the core database
 *
 */
...
/*
 *  builds a requirements graph from the requirements in the core database that are associated with
 *  the current project
 */
...
/*
 *  uses the PDFExporter class to create a report for the current project rating
 *  and returns the path towards it
 *
 */
...
/*
 *  uploads a report to the connected project collaboration platform
 *  if necessary, a new report is generated
 *
 */
