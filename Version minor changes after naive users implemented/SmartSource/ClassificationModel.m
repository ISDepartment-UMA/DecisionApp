...
- (ClassificationModel *) initWithProjectID:(NSString *)projectID
{
    ...
    while ((comp = [componentEnumerator nextObject]) != nil) {
        ...
        if (componentWeightedValue < 1.67) {
            [[classification objectAtIndex:2] addObject:[NSArray arrayWithObjects:comp.componentID, comp.name, [NSNumber numberWithDouble:componentWeightedValue], nil]];
        } else if (componentWeightedValue < 2.34) {
            [[classification objectAtIndex:1] addObject:[NSArray arrayWithObjects:comp.componentID, comp.name, [NSNumber numberWithDouble:componentWeightedValue], nil]];
        } else if (componentWeightedValue <= 3.0) {
            [[classification objectAtIndex:0] addObject:[NSArray arrayWithObjects:comp.componentID, comp.name, [NSNumber numberWithDouble:componentWeightedValue], nil]];
        }
    }
...
- (BOOL)ratingCharacteristicsHaveChanged
{
